	...
	try {
		...
		switch (event.type) {
			case 'checkout.session.completed':
				...
				console.log('WEBHOOK RECEIVED:', JSON.stringify(session, null, 2))
				...
				//return new NextResponse(null, { status: 200 }) // OK response
				break
		...
		return new NextResponse(null, { status: 400 }) // BAD Request response
	} catch (error) {
